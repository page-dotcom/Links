...
export default function Home() {
  ...
  const processLink = async () => {
    if (!url.trim()) return triggerToast("Masukkan URL dulu!");
    setLoading(true);
    ...
    if (!error) {
      setHasil(fullLink);
      setShowResult(true);
      const newRecent = [{ original: url, short: fullLink, slug }, ...recentLinks].slice(0, 5);
      setRecentLinks(newRecent);
      localStorage.setItem('recentLinks', JSON.stringify(newRecent));
      triggerToast("Link Berhasil!");
    }
    setLoading(false);
  };
  ...
  return (
    <>
          ...
          <div className="input-wrapper">
            {!showResult ? (
              ...
              <div id="state-result" className="result-box" style={{ display: 'flex' }}>
                ...
                <button className="btn-black copy" style={{ background: 'var(--accent)' }} onClick={() => {navigator.clipboard.writeText(hasil); triggerToast("Copied!")}}>
                  Copy Link
                </button>
              ...
              {recentLinks.map((item, index) => (
                <div key={index} className="link-row">
                  ...
                  <div className="actions">
                    <button className="btn-icon" title="Copy" onClick={() => {navigator.clipboard.writeText(item.short); triggerToast("Copied!")}}>
                      <span className="material-symbols-rounded">content_copy</span>
                    </button>
                    <Link href={`/stats?slug=${item.slug}`} className="btn-icon" title="Analytics">
                      <span className="material-symbols-rounded">bar_chart</span>
                    </Link>
                    <button className="btn-icon" title="Delete" onClick={() => deleteLink(index)} style={{ color: 'red' }}>
      ...
      {/* Toast Notif */}
      <div className={`toast ${toast.show ? 'show' : ''}`} style={{ position: 'fixed', bottom: '20px', right: '20px', background: '#000', color: '#fff', padding: '10px 20px', borderRadius: '10px', zIndex: 9999 }}>
        {toast.msg}
      </div>
    </>
  );
